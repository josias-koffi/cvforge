...
function makeStoredApplication(overrides: Partial<StoredApplication> = {}): StoredApplication {
  return {
    ...
    rawOfferText: "We are hiring a senior TypeScript developer with 5 years of experience. Skills: TypeScript, Node.js, React. Responsibilities: Build APIs and UIs.",
...
function makeRequest(overrides: Partial<CvGenerationRequest> = {}): CvGenerationRequest {
  return {
    localFields: {
    ...
    promptProfile: {
      ...
      profileSections: {
        certifications: [],
        education: [{ degree: "Master Informatique", honors: "", institution: "Sorbonne", year: "2018" }],
        experiences: [{ company: "Tech Corp", period: "2020-2023", results: "Delivered key APIs", role: "Backend Developer" }],
        interests: "",
        personalProjects: [],
...
const VALID_CV_JSON: CVDocumentContent = {
  ...
  certifications: [],
  education: [{ degree: "Master Informatique", institution: "Sorbonne", mention: "", year: "2018" }],
  experiences: [{ achievements: ["Delivered key APIs"], company: "Tech Corp", description: "Built backend systems", endDate: "2023", position: "Backend Developer", startDate: "2020" }],
  languages: [],
  projects: [],
...
describe("CvGenerationService", () => {
  ...
  describe("generateCv", () => {
    it("calls openRouter.chat with pseudonymised profile (no lastName/phone/email)", async () => {
      ...
      const [messages] = (openRouter.chat as ReturnType<typeof vi.fn>).mock.calls[0] as [Array<{ role: string; content: string }>];
      const userMessage = messages.find((m) => m.role === "user")!;
      const payload = JSON.parse(userMessage.content) as { pseudonymisedProfile: { identity: Record<string, unknown> } };
      ...
      expect(payload.pseudonymisedProfile.identity).toHaveProperty("candidateToken", "[CANDIDATE]");
    });

    it("injects localFields into the returned cvContent", async () => {
      const cvContent = await service.generateCv("user@test.example", "app-001", makeRequest());
    ...
    it("throws NotFoundException when application not found", async () => {
      (store.findByIdForUserEmail as ReturnType<typeof vi.fn>).mockReturnValue(null);
    ...
    it("parses fenced json block from AI response", async () => {
      openRouter.chat.mockResolvedValue("```json\n" + JSON.stringify(VALID_CV_JSON) + "\n```");
    ...
    it("system prompt mentions [CANDIDATE] token and forbids phone/email", async () => {
      await service.generateCv("user@test.example", "app-001", makeRequest());
      const [callMessages] = (openRouter.chat as ReturnType<typeof vi.fn>).mock.calls[0] as [Array<{ role: string; content: string }>];
    ...
    it("normalises partial AI JSON with missing candidate fields gracefully", async () => {
      openRouter.chat.mockResolvedValue(JSON.stringify({
        candidate: {},
        experiences: [{ company: "Acme", position: "Dev", startDate: "2020", endDate: "2023", description: "Work", achievements: [] }],
        skills: { hard: ["Go"], soft: [] },
      }));
    ...
    it("handles non-array achievements in experience items", async () => {
      openRouter.chat.mockResolvedValue(JSON.stringify({
        ...
        experiences: [{ company: "Acme", position: "Dev", startDate: "2020", endDate: "2023", description: "Work", achievements: "not an array" }],
      }));
  ...
  describe("getCvContent", () => {
    ...
    it("returns the stored cvContent", () => {
      const app = makeStoredApplication({ cvContent: VALID_CV_JSON, cvGeneratedAt: "2026-04-20T12:00:00.000Z" });
      (store.findByIdForUserEmail as ReturnType<typeof vi.fn>).mockReturnValue(app);
    ...
    it("throws NotFoundException when application not found", () => {
      ...
      expect(() => service.getCvContent("user@test.example", "missing")).toThrow(NotFoundException);
    });
  });
});
