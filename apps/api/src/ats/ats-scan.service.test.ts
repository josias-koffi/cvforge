...
import { buildPdf, pdfFile } from "./testing/build-pdf";
...
function scanRequest(
  overrides: Partial<Parameters<AtsScanService["scanPublic"]>[0]> = {},
) {
  return {
    file: makeFile(),
    ip: "203.0.113.7",
...
describe("AtsScanService", () => {
  ...
  describe("what it refuses", () => {
    ...
    it("names the refusal with a code", async () => {
      ...
      await expect(
        ...
        response: { code: "CV_FILE_REQUIRED" },
      });
    });
  ...
  describe("the daily budget", () => {
    ...
    it("names the exhausted budget with a code", async () => {
      service = new AtsScanService(
        store,
        impact as unknown as AtsImpactService,
        1,
      ...
      await expect(service.scanPublic(scanRequest())).rejects.toMatchObject({
