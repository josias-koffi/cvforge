...
/**
 * Vendored from ElevenLabs UI (https://ui.elevenlabs.io/docs/components/orb),
 * MIT, via `npx @elevenlabs/cli components add orb`.
 *
 * Kept as delivered apart from one change: the Perlin noise texture is served
 * from `/perlin-noise.png` instead of ElevenLabs' CDN, so an interview does
 * not depend on a third-party host and nothing about our users' sessions
 * leaks to one. See ADR-011.
 *
 * Treat this file as third party: change the wrapper in
 * `components/interview/studio/voice-orb.tsx`, not the shader.
 */
...
function Scene({
  ...
  // Local copy of the upstream texture — see the note at the top of the file.
